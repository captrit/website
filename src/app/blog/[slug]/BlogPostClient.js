...
export default function BlogPostClient({ post, relatedPosts = [], breadcrumbJsonLd, blogPostingJsonLd }) {
  ...
  // If post is not available, show loading state
  if (!post) {
    return (
        ...
        >
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <motion.div variants={itemVariants} className="mb-8">
              <Link 
                href="/blog"
                className="inline-flex items-center gap-2 text-gray-500 hover:text-brand-600 transition-colors group text-sm font-medium"
              >
            ...
            <motion.article variants={itemVariants} className="mb-12">
              ...
              {post?.category && (
                <motion.div 
                  ...
                  className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-brand-50 border border-brand-100 mb-6"
                >
                  <Tag className="w-3 h-3 text-brand-600" />
                  <span className="text-brand-600 text-xs font-semibold uppercase tracking-wide">{post.category}</span>
                </motion.div>
              )}
                ...
                <div className="prose prose-lg max-w-none">
                  {displayContent ? (
                    ...
                    <div className="text-center py-12">
                      <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-brand-500/20 to-brand-600/20 rounded-full flex items-center justify-center">
                  ...
                  {relatedPosts.slice(0, 3).map((relatedPost, index) => (
                    ...
                    >
                      <Link href={`/blog/${relatedPost.slug}`} className="block h-full">
                        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden h-full transform transition-all duration-300 hover:border-brand-300 hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
                          {/* Image */}
                          <div className="relative h-48 w-full overflow-hidden">
                            {relatedPost.cover ? (
                              ...
                              <div className="w-full h-full bg-gradient-to-br from-brand-500/20 to-brand-600/20 flex items-center justify-center">
                                <svg className="w-12 h-12 text-brand-400/30" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          {/* Content */}
                          <div className="p-6">
                            <h3 className="text-lg font-bold mb-3 text-gray-900 group-hover:text-brand-600 transition-colors line-clamp-2">
                              {relatedPost.title}
                            </h3>
                            ...
                            <div className="flex justify-end">
                              <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center group-hover:bg-brand-500 transition-colors">
