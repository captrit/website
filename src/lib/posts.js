...
export function getPostsByYear(year) {
  const allPosts = getSortedPostsData();
  if (year === 'All') return allPosts;
  return allPosts.filter(post => new Date(post.date).getFullYear().toString() === year);
}
...
export function getRelatedPosts(currentPost, limit = 3) {
  ...
  const scoredPosts = otherPosts.map(post => {
    ...
    // Same year gets small score
    if (new Date(post.date).getFullYear() === new Date(currentPost.date).getFullYear()) {
      score += 2;
    }
